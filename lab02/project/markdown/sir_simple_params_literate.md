```@meta
EditURL = "../scripts/sir_simple_params_literate.jl"
```

# Модель SIR с изменёнными параметрами

Здесь рассматривается тот же процесс, но с другим набором параметров,
чтобы показать влияние параметров на динамику эпидемии.

````@example sir_simple_params_literate
using Pkg
